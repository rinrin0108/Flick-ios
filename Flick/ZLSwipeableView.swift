...
func flick(){
    ...
    // Sessionを生成.
    var mySession:NSURLSession = NSURLSession(configuration: myConfig, delegate: ZLSwipeableView(), delegateQueue: nil)
    // 通信先のURLを生成.
    let myUrl:NSURL = NSURL(string: "http://52.8.212.125:3000/flick")!
    println(myUrl)
    ...
    let str:NSString = "{\"jsondata\":{\"userid\":\"" + appDelegate._userid! + "\",\"location\":[" + appDelegate._lat! + "," + appDelegate._lng! + "],\"data\":\"textdata\"}}"
...
public class ZLSwipeableView: UIView ,NSURLSessionDelegate,NSURLSessionDataDelegate{
    // MARK: - Public
    // MARK: Data Source
    ...
    //ここでflickの動きを規定している。種類は以下のいずれか
    //[.None, .Left, .Up, .Right, .Down, .Horizontal, .Vertical, .All]
    public var direction = ZLSwipeableViewDirection.Up
